...
secure_mapping_path <- here("Security", "VBD_patient_id_mapping.csv")

# Store this mapping to a highly secure location, NOT accessible to your dashboard
write_csv(VBD_patientid_maps, secure_mapping_path)
...
invalid_date_data_output_path <- here::here("data", "review", "VBD_invalid_dates.csv")


raw_parsed_dates <- fix_dates(VBD_raw_PHI_data)
...
if (nrow(raw_invalid_dates) > 0){
  ...
  message(
    "IMPORTANT: Please open the data / review / 'VBD_invalid_dates.csv'
...
VBD_deidentified_cleaned <- joined_deidentified(raw_valid_dates, VBD_patientid_maps)




# Save the de-identified dataset to a location accessible by your dashboard
deidentified_data_output_path <- here::here("data","clean", "VBD_deidentified_cleaned.csv")


readr::write_csv(VBD_deidentified_cleaned, deidentified_data_output_path)
message(paste("De-identified data saved to:", deidentified_data_output_path))


# 4. Save Cleaned Data
#----------------------------------------------
qsave(VBD_deidentified_cleaned, here::here("data","clean", "VBD_deidentified_cleaned.qs"))
